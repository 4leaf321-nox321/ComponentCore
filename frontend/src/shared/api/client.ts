...
/**
 * 바이너리 응답(glTF · STEP)을 Blob 으로. 3D 뷰어가 `URL.createObjectURL` 로 읽는다.
 * **평범한 링크로는 안 된다** — access 토큰은 메모리에만 있어 `<a href>` 에는 실리지 않는다.
 */
export async function fetchBlob(path: string, init?: RequestInit): Promise<Blob> {
  let response = await send(path, init)
  if (response.status === 401 && !path.startsWith('/auth/')) {
    if (await tryRefresh()) response = await send(path, init)
    else onSessionLost?.()
  }
  if (!response.ok) throw await parseError(response)
...
/** JSON 본문을 보내고 Blob 을 받는다(CAD 미리보기). */
export function postForBlob(path: string, body: unknown): Promise<Blob> {
  return fetchBlob(path, { method: 'POST', body: JSON.stringify(body ?? {}) })
}
