...
test('도면의 좌표계를 더하고 원점에 **치수 식**을 적는다 — DOE 로 치수가 바뀌면 따라간다', () => {
  ...
  const { rerender } = render(<FramesDialog open frames={frames} onChange={(next) => (frames = next)} onClose={() => {}} />)

  fireEvent.click(screen.getByRole('button', { name: '좌표계 추가' }))
  expect(frames).toEqual([{ name: '좌표계 1', origin: [0, 0, 0], rotate: [0, 0, 0] }])
  rerender(<FramesDialog open frames={frames} onChange={(next) => (frames = next)} onClose={() => {}} />)
...
test('전역(global)이나 겹치는 이름은 알린다', () => {
  render(<FramesDialog open frames={[{ name: 'global' }]} onChange={() => {}} onClose={() => {}} />)
