...
export const OP_SPECS: OpSpec[] = [
  ...
  },
  {
    ...
    label: '볼트',
    group: '입체',
    help: '머리 + 와셔 + 몸통. 「기준 자리」 는 머리가 앉는 면의 점, 몸통은 아래(-Z)로. 나사산은 없고 ISO 비례.',
    fields: [
      { key: 'at', label: '머리가 앉는 점', kind: 'xyz' },
      ...
      { key: 'down', label: '몸통이 아래(-Z)로', kind: 'checkbox' },
    ],
  ...
  },
  {
    ...
    label: '위치 핀',
    group: '입체',
    help: '원기둥에 끝 모따기. 「기준 자리」 는 밑면 중심, 위(+Z)로 선다.',
    fields: [
  ...
  },
  {
    ...
    label: '스페이서',
    group: '입체',
    help: '가운데 구멍 난 원통(스탠드오프). 「기준 자리」 는 밑면 중심.',
    fields: [
      { key: 'at', label: '밑면 중심', kind: 'xyz' },
  ...
  },
  {
    ...
    label: '묶음',
    group: '조합',
    help: '여럿을 붙이지 않고 한 묶음으로 — 조립의 마지막 줄. 합치기(union)와 달리 덩어리가 따로 남는다.',
  ...
  },
  {
    ...
    label: '가져오기',
    group: '조합',
    help: '다른 도면(부품 · 지그)을 통째로 가져다 놓는다. 가져온 쪽의 변수를 덮어써 조립의 변수로 움직일 수 있다 — 보통 「조립」 화면에서 다룬다.',
    fields: [
      { key: 'source', label: '가져올 도면 (part:… · jig:… · work:…)', kind: 'text' },
      { key: 'translate', label: '자리', kind: 'xyz' },
