...
test('치수를 고치면 레시피의 params 가 바뀐다 — 그것을 쓰는 칸은 서버가 푼다', () => {
  render(<Host />)
  fireEvent.change(screen.getByLabelText('치수 판_길이'), { target: { value: '120' } })
  expect(JSON.parse(screen.getByTestId('json').textContent!).params).toEqual({ 판_길이: 120 })
})

test('치수 이름을 바꾸면 그것을 쓰던 식도 따라 바뀐다 — 안 그러면 레시피가 깨진다', () => {
  render(<Host />)
  fireEvent.blur(screen.getByLabelText('치수 이름 판_길이'), { target: { value: '길이' } })
...
test('치수를 더하고 지운다', () => {
  ...
  fireEvent.change(screen.getByLabelText('새 치수 이름'), { target: { value: '두께' } })
