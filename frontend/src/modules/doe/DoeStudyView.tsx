...
import {
  ...
  PointsNav,
  pointRowProps,
} from "@/modules/doe/PointsGallery";
...
export function DoeStudyView({
  ...
  const names = study.factors
    .filter((one) => one.mode !== "fixed")
    .map((one) => one.name);
  const rows = study.points;
  const viewable = study.points
  ...
  return (
    <div className="space-y-4">
      {/*
        설계점 파일이 치워졌을 때. **이력이 남아 있다는 말이 헛말이 되지 않게** 여기서 길을
        준다 — 표와 3D 는 스냅샷으로 그대로 뜨지만(3D 는 레시피로 다시 만든다) STEP 은
        없으므로 「보내기」 가 막힌다. 그 사실과 할 일을 한자리에서 말한다.
      */}
        ...
        <div className="flex flex-wrap items-center gap-2 rounded-md border border-amber-300 bg-amber-50 p-3 dark:border-amber-700 dark:bg-amber-950/40">
          ...
          <div className="min-w-0">
            <p className="text-xs font-medium">
              설계점 파일이 보관 기한을 지나 정리되었습니다
            </p>
            <p className="text-muted-foreground text-xs">
              설정(레시피 · 인자 · 시드 · 조건)은 그대로 남아 있습니다 — 「다시
              만들기」 를 누르면 같은 것이 다시 납니다.
            </p>
          </div>
          <Button
            size="sm"
      ...
      <div className="bg-muted/40 flex flex-wrap items-center gap-2 rounded-md border p-3">
        ...
        <div className="min-w-0">
          {study.exported_at ? (
            <>
              <p className="text-xs font-medium">
                공유 폴더에 보냈습니다 ({shownDateTime(study.exported_at)}) —
                해석은 이 폴더를 읽습니다
              </p>
              <p className="truncate font-mono text-xs">
                {study.export_dir_windows}
              </p>
            </>
          ) : (
            <>
              ...
              <p className="text-muted-foreground text-xs">
                {!finished
                  ? "다 만들어지면 공유 폴더로 보낼 수 있습니다."
                  : study.local_ready
                    ? "「공유 폴더로 보내기」 를 누르면 해석이 읽는 폴더에 복사됩니다."
                    : "보낼 파일이 없습니다 — 「다시 만들기」 를 먼저 누르세요."}
              </p>
            </>
          )}
        </div>
        ...
        <Button
          ...
          className="ml-auto"
          disabled={
            !finished || study.done === 0 || !study.local_ready || exporting
          }
          onClick={() => void send()}
        >
      ...
      <div className="flex flex-wrap items-center gap-2 text-sm">
        ...
        <span>
          ...
          {study.failed > 0 && (
            <span className="text-destructive"> · 실패 {study.failed}</span>
          )}
        </span>
        {/*
          실패한 점만 한 번 더. 범위를 고쳐 다시 돌리는 것이 아니라 **같은 값으로** 다시
          해 보는 것이다 — 범위를 고칠 생각이면 새 스터디를 만든다.
        */}
          ...
          {galleryMode === "single" ? (
            ...
            <label className="flex items-center gap-2 text-xs">
              <input
                ...
                checked={
                  viewable.length > 0 &&
                  viewable.every((n) => picked.includes(n))
                }
                onChange={(event) =>
                  setPicked(event.target.checked ? viewable : [])
                }
                aria-label="전체 선택"
              />
            ...
            <Table className="min-w-max whitespace-nowrap">
              <TableHeader>
                <TableRow>
                  ...
                  {hasInterference && (
                    <TableHead title="구성품끼리 겹침 — 조립일 때">
                      간섭
                    </TableHead>
                  )}
                    ...
                    >
                      {picking && (
                        <TableCell onClick={(event) => event.stopPropagation()}>
                          {row.viewable && (
                            <input
                              type="checkbox"
                              checked={row.isPicked}
                              onChange={row.toggle}
                              aria-label={`p${String(point.number).padStart(4, "0")} 고르기`}
                            />
                          )}
                        </TableCell>
                      )}
                          ...
                          {point.interference ? (
                            point.interference.ok ? (
                              ...
                              <span
                                ...
                                title={point.interference.items
                                  .filter((one) => !one.ok)
                                  .map(
                                    (one) =>
                                      `${one.a} × ${one.b} ${one.volume} mm³`,
                                  )
                                  .join("\n")}
                              >
                                {
                                  point.interference.items.filter(
                                    (one) => !one.ok,
                                  ).length
                                }
                                건
                      ...
                      <TableCell className="text-xs">
                        {point.status === "ok" ? (
                          ...
                          <span title={point.error}>실패</span>
                        ) : (
                          "기다리는 중"
                        )}
                      </TableCell>
