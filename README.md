...
- **내 작업** — 나만 보는 문서. 머리에는 작업 자체의 일(실험계획 만들기 · 공용 부품/지그로 승격 ·
  ...
  안 「파일」 탭에서 — 올린 STEP 이 새 버전이 된다. 부품 작업에는 지그 만들어 주기 탭이 덤으로 있다.
- **실행 기록** — 내가 건 작업(부품 평가 · 지그 생성)과 산출물.
- **실험계획(DOE)** — 독립 공간. 대상(부품 · 지그 · 조립)을 고르고 변수에 범위를 주면 인스턴스를
  여럿 만들어 공유 폴더에 STEP + 표로 쏟는다. 해석(ANSYS)은 그 폴더를 읽는다. 전체 조합 ·
  LHS(시드로 재현) · CSV. 표에는 바꾼 변수와 파일 이름만 — 질량 · 크기는 계산하지 않는다
  (결과는 해석이 낸다). 값은 인자마다 고른 가공 단위(기본 0.1 mm)로 맞춘다. 한 번에 만드는
  설계점 상한은 관리자가 「서버 › 설정」 에서 바꾼다.
