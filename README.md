...
- **실험계획(DOE)** — 치수에 범위를 주면 형상을 여러 벌 만들어 공유 폴더에 STEP + 표로 쏟는다.
  해석(ANSYS)은 그 폴더를 읽는다. 전체 조합 · LHS(시드로 재현) · 부등식 필터 · CSV.
...
- **부품이든 지그든 그리는 방법은 같다** — 레시피 하나. 작업의 **종류**(부품 · 지그)가 어느
  카탈로그로 올라갈지와 덤으로 쓰는 도구를 정한다: 부품 작업엔 **지그 생성기**(제품에서 자동
  배치), 지그 작업엔 **잡는 부품** 잇기. 변수 · 실험계획은 둘 다 그대로 쓴다.
