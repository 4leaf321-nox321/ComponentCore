...
# 데이터 소스 동기화 타이머 — **이 플랫폼에는 없다**(주기로 돌 것이 없다. 부품 평가 · 지그
# 생성 · 실험계획은 사람이 걸고 워커가 집는다). 틀에서 온 자리만 남겨 둔다 — 템플릿이 없으니
# 켜도 건너뛴다.
...
ensure_dirs() {
    ...
    if [[ -n "$DOE_HOST_DIR" ]]; then
        # 마운트가 아직 안 올라온 자리에 폴더만 만들면, 해석 쪽은 빈 폴더를 보고 우리는 쓴 줄 안다.
        [[ -d "$(dirname "$DOE_HOST_DIR")" ]] \
            || err "실험계획 공유 폴더의 상위가 없습니다: $(dirname "$DOE_HOST_DIR") — 마운트를 먼저 올리세요"
        mkdir -p "$DOE_HOST_DIR"
        chown "$OPERATOR:$OPERATOR" "$DOE_HOST_DIR" 2>/dev/null \
            || warn "공유 폴더의 소유자를 못 바꿨습니다($DOE_HOST_DIR) — 네트워크 마운트면 정상입니다. 쓸 수 있는지는 install 끝에 확인하세요."
    fi
    instance_save
    ha_save
}
...
SQL
    ...
    if [[ -n "$DOE_HOST_DIR" ]]; then
        # 컨테이너 안 경로다 — 유닛이 $DOE_HOST_DIR 를 /data/doe-export 에 건다.
        sed -i -e "s|^# *DOE_EXPORT_ROOT=.*|DOE_EXPORT_ROOT=/data/doe-export|" "$ENV_FILE"
    fi
    if [[ -n "$HA_ROLE" ]]; then
...
render_unit_paths() {  # $1=template
    ...
    if [[ -n "$DOE_HOST_DIR" ]]; then
        doe_sed="s|^@@DOE_BIND@@.*|    --bind $DOE_HOST_DIR:/data/doe-export \\\\|"
    else
        doe_sed='/^@@DOE_BIND@@/d'
    fi
...
check_doe_dir() {
    ...
    if as_op sh -c "touch '$probe' && rm -f '$probe'" 2>/dev/null; then
...
cmd_status() {
    ...
    echo "== 실험계획 공유 폴더 =="
    if [[ -n "$DOE_HOST_DIR" ]]; then
        echo "  $DOE_HOST_DIR → 컨테이너의 /data/doe-export"
