...
#
# **저장소는 공개다 — 토큰 없이 그냥 받아진다.** 토큰은 선택이다:
#
#   GH_TOKEN=ghp_xxx ./fetch-release.sh     비공개 포크, 또는 API 호출 한도에 걸릴 때
#
# 토큰이 있으면 **애셋 API** 로 받는다(`/releases/assets/<id>` + octet-stream). 비공개
# 저장소에서는 browser_download_url 이 인증을 받지 않아, 로그인 페이지의 HTML 이 tar.gz
# 라는 이름으로 저장되고 그 사실은 서버에서 푸는 순간에야 드러나기 때문이다.
set -euo pipefail
...
if ! RELEASE="$(api "$REL_URL" 2>/dev/null)"; then
    echo "릴리스를 읽지 못했습니다 — 아직 릴리스가 없거나, 저장소 이름 · 버전이 다릅니다:"
    echo "    https://github.com/$REPO/releases"
    echo "  비공개 포크라면 토큰을: GH_TOKEN=<개인 토큰> $0 ${VERSION:+-v $VERSION}"
    exit 1
fi
