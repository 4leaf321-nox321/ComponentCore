#!/usr/bin/env bash
# 백업 — 데이터베이스와 **작업물**(STEP · glTF · 올린 STEP)을 같은 시각에 함께 받는다.
#
...
#
# **둘 중 하나만 받으면 복구되지 않는다.** DB 에는 도면(레시피) · 작업 · 작업물의 경로가,
# filestore 에는 그 실제 파일(STEP · glTF)이 있다. 시점이 어긋나면 「DB 에는 있는데 파일이
# 없는」 행이 생기고, 그 상태로 앱이 뜬다 — 목록은 멀쩡하고 3D 를 열 때만 터진다.
#
# **실험계획의 공유 폴더는 여기서 안 받는다.** 그 폴더는 해석 쪽과 함께 쓰는 자리라 주인이
# 따로 있고, 안에 든 STEP 은 도면과 설계점에서 언제든 다시 만들 수 있다.
#
# ## 배치
#
#   <BackupRoot>/db/db-<시각>.dump   pg_dump 커스텀 포맷 — 일 7벌 + 일요일분 4벌
#   <BackupRoot>/filestore/          작업물 미러 1벌 (STEP · glTF)
#   <BackupRoot>/env/.env            접속 정보·JWT 비밀키
#   <BackupRoot>/LAST_BACKUP.txt     무엇을 언제 받았는지
#
# 작업물은 **불변 파일**이라 세대가 필요 없다 — 미러 한 벌이면 된다. 덤프만 세대를 둔다.
#
...
# 통째로 담는다. 큰 것(형상 파일)은 DB 가 아니라 filestore 에 있고, DB 에 든 것은 도면 ·
# 치수표 · 설계점 같은 JSON 이라 덤프가 무겁지 않다.
...
FILES=0
if [[ -d "$FILESTORE" ]]; then
    info "작업물 미러: $FILESTORE -> $BACKUP_ROOT/filestore"
    mkdir -p "$BACKUP_ROOT/filestore"
    # --delete 로 미러를 맞춘다. 작업물은 불변이라 세대가 필요 없다.
    rsync -a --delete "$FILESTORE/" "$BACKUP_ROOT/filestore/"
    FILES=$(find "$BACKUP_ROOT/filestore" -type f | wc -l)
else
    echo "경고: filestore 가 없습니다 — 아직 만든 형상이 없다면 정상입니다." >&2
fi
