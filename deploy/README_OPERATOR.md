...
#  app.service.template  worker.service.template  mcp.service.template
...
1. `~/apps/<slug>/{filestore,logs}` 생성 (공용 스토리지 `DATA_DIR` 를 주면 작업물 · `.env` · 백업은 거기 — 「8. 이중화」)
   그리고 `DOE_HOST_DIR` 을 줬으면 실험계획 공유 폴더도 — 「5c」
...
**DB 와 작업물(STEP · glTF)을 같은 시각에 함께 받는다.** 둘 중 하나만 받으면 복구되지 않는다.
...
DB 를 지우고 다시 만들며 작업물도 지운다. `.env`·DB 역할·systemd 유닛은 남는다.
...
## 5b. MCP 서버 (AI 연동 — Claude · Gemini · Codex, 선택)

`deploy.sh install`/`update` 가 **MCP 서버까지 자동 설치**한다 — 별도 venv 생성 + (번들에
동봉된 휠로) **오프라인 pip 설치** + `<slug>-mcp` systemd 서비스 기동까지. 사용자는 Claude
Code · Claude Desktop · Gemini CLI 에서 **부품을 그리고 지그를 만들고 실험계획을 돌릴 수 있다**
(도구 40여 개 — 도면 · 그림 · 치수 재기 · 지그 생성 · 조립 · DOE · 승격).
...
- **사용자 등록(각자)** — 화면의 **「내 정보」 에서 토큰을 발급하면 그 자리에 도구별 등록
  방법이 토큰까지 채워져 나온다**(Claude Code · Claude Desktop · Gemini CLI · Codex CLI ·
  스크립트). 운영자가 따로 안내할 것이 없다. 명령만 보면:
  ```bash
...
- **화면이 안내하는 주소를 맞춘다.** 「내 정보」 는 기본적으로 *접속한 호스트 + (앱 포트 + 2)* 로
  등록 명령을 만든다. 프록시 뒤라 사용자가 다른 주소로 들어오면 `.env` 에
  `MCP_PUBLIC_URL=http://<서버>:<mcp포트>/mcp` 를 적는다(앱 재시작).
...
## 5b-2. 작업 워커 — **형상을 만드는 것이 여기다**

`deploy.sh install`/`update` 가 `<slug>-worker.service` 를 함께 설치한다. 도면을 저장하면 **형상
평가**가, 「부품에서 지그 생성」 이 **지그 생성**이, 실험계획이 **설계점 수십 벌**이 전부 작업이
되고 이 워커가 집어 돌린다 — 앱과 **같은 SIF · 같은 .env**, `python -m app.worker`.

- **없으면 모든 것이 「대기」 다.** 도면을 저장해도 3D 가 안 나오고, 지그 생성도 안 끝난다.
  `sudo systemctl status <slug>-worker` / 로그 `journalctl -u <slug>-worker -f`.
- **CPU 를 쓴다.** 형상 하나를 만드는 데 수백 ms ~ 수 초, 실험계획은 설계점 수만큼 곱해진다.
  200 점짜리 DOE 하나가 워커를 몇 분에서 수십 분 잡는다 — 그동안 다른 작업은 줄을 선다.
- **이중화면 A · B 에 하나씩 뜬다.** 같은 작업을 두 번 안 집는다(DB 의 `SKIP LOCKED`). 한 대가
  죽으면 그 대가 하던 작업은 5분 뒤 다른 대가 되살린다.
- `update` 는 앱과 워커를 함께 멈춘다(하던 작업은 최대 2분 기다린다) — 옛 코드가 새 마이그레이션
  위에서 작업을 집으면 안 된다. **DOE 가 도는 중에 갱신하면** 그 스터디는 중간에서 끊기고, 다시
  만들어야 한다(만들어진 점까지는 폴더에 남는다).

## 5c. 실험계획(DOE)의 공유 폴더 — **이 플랫폼만의 것**

실험계획은 설계점마다 STEP 과 `manifest.csv` 를 **공유 폴더**에 쏟는다. 해석(ANSYS)을 도는 쪽이
그 폴더를 그대로 읽는다 — 그래서 이 폴더는 앱의 자료가 아니라 **두 시스템이 함께 쓰는 자리**다.
...
- 유닛이 이 폴더를 컨테이너의 `/data/doe-export` 에 걸고, `.env` 의 `DOE_EXPORT_ROOT` 가 그것을
  가리킨다. **앱과 워커 둘 다** 건다(앱은 만들기 전에 쓸 수 있는지 보고, 실제로 쓰는 것은 워커다).
- **안 정해도 DOE 는 만들어진다.** 설계점 STEP 은 서버 보관 폴더에 쌓이고, 막히는 것은 해석으로
  **「보내기」** 하나다 — 그때 「공유 폴더에 쓸 수 없습니다」 로 거절한다. 조용히 컨테이너 안에
  쓰지 않기 때문이다(그러면 이미지를 바꾸는 날 결과가 통째로 사라진다). 폴더를 나중에 정하면
  **이미 만들어 둔 것도 그대로 보낼 수 있다** — 원본이 보관 폴더에 있다.
...
| 설치 경로 | `~/apps/<slug>` |
| 데이터베이스 · 역할 | `<slug>` |
| systemd 유닛 | `<slug>.service` · `<slug>-worker.service` · `<slug>-mcp.service` |
...
사용자 · 외부 AI ──HTTPS──▶ 메인 서버 (포탈 + nginx — 메인 서버 쪽이 관리)
                    ...
                    PostgreSQL 주 ── 복제 ──▶ 대기          DB VIP(있으면) 가 주를 따라간다
                         └──── /data/<slug>/ 공용 ────┘      작업물 · 백업 · .env
```

| 무엇이 어디에 | |
| --- | --- |
| 코드(SIF) · 로그 · MCP venv | 각 서버 `~/apps/<slug>` (`INSTALL_DIR`) |
| 작업물 · 백업 · `.env` · 복제 비밀번호 | `/data/<slug>/{filestore,backup,.env,db/}` (`DATA_DIR` — 경로는 env) |
| 실험계획 공유 폴더 | `DOE_HOST_DIR` (해석과 함께 쓰는 자리 — 「5c」). `DATA_DIR` 를 주면 그 아래 `doe-export` 가 기본 |
...
**메인 서버가 아직 없어도** A · B 설치와 8.3 · 8.4 의 리허설은 전부 된다 — 화면은 `http://<A의 IP>:8060/<slug>/` 로 직접 본다(접두어를 붙여서). http 로 직접 보는 동안은 쿠키에 Secure 가 안 붙을 뿐 로그인 유지는 된다.
