...
CompCore(Component Engineering Core)는 **하나의 독립 플랫폼**이다. StandardPlatform 의
인스턴스(확장 모듈)가 아니라, 그 틀에서 로그인 · 설정 · 오류 규약 같은 제반 구조만 가져와
세운 저장소다.
...
`CCR-<MODULE>-<NNNN>`. 코드 곳곳의 `errors.code()` 가 조립하는 문자열이라 빌드에 박힌다 —
로그 검색이 한 값으로 걸려야 한다.
"""
